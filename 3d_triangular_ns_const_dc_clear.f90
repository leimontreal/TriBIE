...
!
! LZ
! Adding normal stress change(const dc)
! Oct.24,2018
!------------------------------------------------
...
real (DP), DIMENSION(:,:), ALLOCATABLE :: slipz1_inter,slipz1_cos,v_cos,slip_cos,&
      slipz1_tau,slipz1_v,xLf_inter,tau_inter,v_inter
...
if(myid == master)then
   ...
   ALLOCATE (maxv(nmv), &
	tmv(nmv),tas(nas),tcos(ncos))

!!! modify output number
   ALLOCATE (slipz1_inter(Nt_all,nas),slipz1_cos(Nt_all,ncos), &
	           v_cos(Nt_all,ncos),slip_cos(Nt_all,ncos), &
             slipz1_tau(Nt_all,ncos),slipz1_v(Nt_all,ncos),&
            xLf_inter(Nt_all,nas),tau_inter(Nt_all,nas),v_inter(Nt_all,nas))
end if
...
!----------------------------------------------
       ...
       do while(cyclecont) 
!
   call derivs(myid,dydt,2*Nt,Nt_all,Nt,t,yt,x) 
...
!-----Interseismic slip every ? years----
   ...
   if (t.ge.tslip_ave)then
      ...
      do i=1,Nt_all
        ...
        v_inter(i,ias)=yt_all(2*i-1)*1.d-3/yrs
...
!----velocity in mm/yr ; slip in meter, moment in 10^{14} Nm -- 
  if(myid==master)then 
    ...
    call output(Ioutput,Isnapshot,Nt_all,Nt,hd,imv,ias,icos,x,&
     ...
     xi_all,x_all,slipz1_tau,slipz1_v,xLf_inter,tau_inter,v_inter)         
...
DEALLOCATE (slipz1_inter,slipz1_cos,v_cos,slip_cos,slipz1_tau,slipz1_v,&
           xLf_inter,tau_inter,v_inter)
end if 


DEALLOCATE (stiff,stiff_ns,xLf)
...
!------------------------------------------------------------------------------
!------------------------------------------------------------------------------
subroutine rkqs(myid,y,dydx,n,Nt_all,Nt,x,htry,eps,yscal,hdid,hnext,xx)
Use mpi
...
implicit none
	...
	real (DP) :: dydx(n),y(n),yscal(n),xx(Nt)
       external derivs
...
!     write(*,*)'in rkqs, bf rkck, myid=',myid,y(n-1)
  1    call rkck(myid,dydx,h,n,Nt_all,Nt,y,yerr,ytemp,x,derivs,xx)
...
!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------
subroutine rkck(myid,dydx,h,n,Nt_all,Nt,y,yerr,yout,x,derivs,xx)
USE phy3d_module_non, only :nprocs
implicit none
integer, parameter :: DP = kind(1.0d0)   
integer :: n,i,NMAX,myid,Nt_all,Nt
external derivs
real (DP) :: h,x,dydx(n),y(n),yerr(n),yout(n),xx(Nt)
...
ALLOCATE (ak2(nmax),ak3(NMAX),ak4(NMAX),ak5(NMAX),ak6(NMAX),ytemp(NMAX))
       ...
       call derivs(myid,ak2,n,Nt_all,Nt,x+A2*h,ytemp,xx)

       do  i=1,n
            ytemp(i)=y(i)+h*(B31*dydx(i)+B32*ak2(i))
       end do 
      call derivs(myid,ak3,n,Nt_all,Nt,x+A3*h,ytemp,xx)

       do  i=1,n
            ytemp(i)=y(i)+h*(B41*dydx(i)+B42*ak2(i)+B43*ak3(i))
       end do 
       call derivs(myid,ak4,n,Nt_all,Nt,x+A4*h,ytemp,xx)

       do  i=1,n
            ytemp(i)=y(i)+h*(B51*dydx(i)+B52*ak2(i)+B53*ak3(i)+  &
                    B54*ak4(i))
       end do 
       call derivs(myid,ak5,n,Nt_all,Nt,x+A5*h,ytemp,xx)

       do  i=1,n
            ytemp(i)=y(i)+h*(B61*dydx(i)+B62*ak2(i)+B63*ak3(i)+  &
                  B64*ak4(i)+B65*ak5(i))
       end do 
       call derivs(myid,ak6,n,Nt_all,Nt,x+A6*h,ytemp,xx)
...
!------------------------------------------------------------------------------
!------------------------------------------------------------------------------
subroutine derivs(myid,dydt,nv,Nt_all,Nt,t,yt,x)
USE mpi
...
!open(14,file='asperity.txt',status='old')
!    do i= 1,77 
!     read(14,*) asper(i)
!   end do
!close(14)
...
!       open(177,file='dc.txt',access='append',status='unknown')
          do k=1,Nt_all
...
!               end do     
!               if (asper_L)then
!                   xLf_all(k)=abs(ab(3))*seff_all(k)*hnucl*0.5*(1-xnu)/(gamma*xmu)
!                 else 
...
real (DP) :: slipz1_inter(Nt_all,nas),slipz1_cos(Nt_all,ncos),&
        ...
        xi_all(Nt_all),x_all(Nt_all),xLf_inter(Nt_all,nas),tau_inter(Nt_all,nas),v_inter(Nt_all,nas)
...
if(Ioutput == 0)then    !output during run 
   ...
   if(imv==nmv)then
    ...
    if(ias==nas)then
       ...
       open(35,form='unformatted',file=trim(foldername)//'tau-inter'//jobname,access='append',status='unknown')
       open(36,form='unformatted',file=trim(foldername)//'v-inter'//jobname,access='append',status='unknown')
      ...
      do j=1,nas
        ...
        write(35) tau_inter(i,j)
        write(36) v_inter(i,j)
